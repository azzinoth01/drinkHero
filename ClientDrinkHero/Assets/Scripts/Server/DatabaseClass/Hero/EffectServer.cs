...
#if CLIENT
public class EffectServer : DatabaseItem, IEffect
{
#endif
#if SERVER
public class Effect : DatabaseItem {
    ...
    private static Dictionary<string,EffectServer> _cachedData = new Dictionary<string,EffectServer>();
    ...
    public EffectServer(EffectServer statusEffect) {
    ...
    public static List<EffectServer> CreateObjectDataFromString(string message) {

        List<EffectServer> list = new List<EffectServer>();

        List<string[]> objectStrings = DatabaseItemCreationHelper.GetObjectStrings(message);
        TableMapping mapping = DatabaseManager.GetTableMapping<EffectServer>();

        foreach(string[] obj in objectStrings) {
            EffectServer item = new EffectServer();
            foreach(string parameter in obj) {
                ...
                if(parameterName == mapping.PrimaryKeyColumn) {
                    if(_cachedData.TryGetValue(parameterValue,out EffectServer existingItem)) {
                        item = existingItem;
                        break;
                    }
                    else {
